...
1. `bin/macnotifier` CLI ラッパースクリプト（`open -n` をラップ、シンボリックリンク対応）
2. `scripts/build.sh` 完成（Info.plist 生成、コンパイル、署名、アイコン組み込み）
3. アプリアイコン追加（吹き出し、モノクロ、黄金比グリッド）
...
2. `Info.plist` を生成（`CFBundleIconFile` 含む）
3. `swiftc` でコンパイル
...
2. `./scripts/test.sh` で全テストがパスすること
3. `./bin/macnotifier -m "test"` で通知が表示されること
4. `./bin/macnotifier -m "test" -e "echo clicked > /tmp/test"` でクリック時にコマンド実行
5. `./bin/macnotifier -m "test" -a com.github.wez.wezterm` でクリック時に WezTerm がアクティブ化
6. 通知クリック後にプロセスが終了すること
